...
\Title{A Symmetry-Based Perspective on Knowledge Distillation:
Symmetric versus Asymmetric Approaches for Image Classification}
...
The concept of symmetry has deep roots in deep learning. Convolutional neural networks exploit translational symmetry through weight sharing~\cite{ref13}. Group-equivariant CNNs~\cite{ref14} extend this to broader symmetry groups. Self-attention mechanisms~\cite{ref15} exhibit permutation symmetry. In representation learning, contrasting symmetric and asymmetric paradigms has proven fruitful: SimSiam~\cite{ref16} and BYOL~\cite{ref17} demonstrate that asymmetric architectures can prevent representational collapse. The key insight is that asymmetric design (e.g., a predictor network in SimSiam) creates an information bottleneck that prevents the student from trivially copying the teacher---a principle that may generalize to KD: asymmetric distillation methods may better prevent overfitting to teacher-specific noise.
...
\subsection{Implications for Designing New KD Methods}

The symmetry perspective suggests two design strategies: (1) \textbf{layer-adaptive distillation} could dynamically choose symmetric or asymmetric losses per layer based on the measured similarity between teacher and student features---early layers (high similarity) benefit from symmetric alignment, while deeper layers (lower similarity) may benefit from asymmetric structure preservation; (2) \textbf{hybrid objectives} could combine equivariance and invariance terms with adaptive weighting, potentially achieving both fast convergence and high peak accuracy. These directions are left for future work.
...
SP's superior performance could alternatively be attributed to the regularizing effect of its batch-size-dependent similarity computation rather than its asymmetric nature. When the batch size is small, the similarity matrices may be noisy, potentially reducing SP's effectiveness; when large, the regularization may be stronger. A systematic ablation of batch size would help distinguish these mechanisms. Similarly, KD's temperature parameter $\tau$ controls the softness of the target distribution---higher temperatures emphasize relative class relationships, introducing an element of relational (asymmetric) knowledge into an otherwise symmetric method. These interdependencies highlight that the symmetric/asymmetric distinction, while analytically useful, captures only part of the full picture.

\subsection{Generalizability to Other Domains}

While this study focuses on image classification, the symmetry-based taxonomy is domain-agnostic. In NLP, for example, BERT-based KD methods often use symmetric MSE on hidden states (e.g., DistilBERT) or asymmetric KL divergence on prediction distributions---the same distinction applies. In graph neural networks, node-level feature alignment (symmetric) contrasts with structure-preserving distillation of graph topologies (asymmetric). Extending the taxonomy to these domains is a natural direction for future work.

\subsection{The Symmetric/Asymmetric Distinction in Self-Supervised Learning}

The parallel between KD and self-supervised learning (SSL) merits deeper discussion. SimSiam~\cite{ref16} and BYOL~\cite{ref17} use asymmetric architectures (predictor networks, momentum encoders) to prevent representational collapse. This mirrors our finding that asymmetric KD methods (SP, RKD) use relational structure as a natural regularizer, potentially preventing the student from overfitting to teacher-specific noise. The connection suggests that insights from SSL---particularly the role of asymmetry in preventing collapse---could inform the design of more robust KD methods.

\subsection{Limitations}

(1) Single dataset (CIFAR-100) and architecture family (ResNet). (2) Single-run experiments per method---without multiple seeds, statistical significance cannot be established, and reported accuracy differences (e.g., 0.14\% between KD and SP) may lie within the noise floor. (3) Hyperparameters adopted from prior literature without per-method tuning, which may disadvantage some methods. (4) Limited method coverage (six methods); influential approaches such as Contrastive Representation Distillation~\cite{ref18} and Born-Again Networks~\cite{ref22} were not included due to computational constraints. (5) The taxonomy is descriptive rather than predictive; it organizes known behaviors but does not guarantee generalization to unseen settings. (6) Additional metrics beyond top-1 accuracy (e.g., per-class accuracy, calibration error) would enrich the comparison.
...
\bibitem{ref21} Gou, J.; Yu, B.; Maybank, S.J.; Tao, D. Knowledge distillation: A survey. \textit{International Journal of Computer Vision} \textbf{2021}, \textit{129}, 1789--1819.
\bibitem{ref22} Furlanello, T.; Lipton, Z.; Tschannen, M.; Itti, L.; Anandkumar, A. Born-again neural networks. In \textit{Proceedings of the International Conference on Machine Learning}, PMLR, 2018; pp.~1607--1616.
\end{thebibliography}
